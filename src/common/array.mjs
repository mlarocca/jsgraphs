...
import { isNumber, randomInt } from './numbers.mjs';
import { setDifference } from './set.mjs';
import {
  ERROR_MSG_INVALID_ARGUMENT,
  ERROR_MSG_ARGUMENT_TYPE,
  ERROR_MSG_PARAM_EMPTY_ARRAY
} from './errors.mjs';
import { consistentStringify } from './strings.mjs';
...
export function mean(values) {
  if (!Array.isArray(values) || !values.every((v) => isNumber(v))) {
    throw new TypeError(
      ERROR_MSG_ARGUMENT_TYPE('mean', 'values', values, 'Array<number>')
    );
  }
  let n = values.length;
  if (n === 0) {
...
export function variance(values) {
  if (!Array.isArray(values) || !values.every((v) => isNumber(v))) {
    throw new TypeError(
      ERROR_MSG_ARGUMENT_TYPE('variance', 'values', values, 'Array<number>')
    );
  }
  let n = values.length;
  if (n === 0) {
...
export function zip(a1, a2) {
  ...
  if (a1.length !== a2.length) {
    throw new TypeError(
      ERROR_MSG_INVALID_ARGUMENT('zip', '(a1,a2)', `(${a1},${a2})`)
    );
  }
...
export function compareAsSets(array1, array2, key = consistentStringify) {
  if (!Array.isArray(array1)) {
    throw new TypeError(
      ERROR_MSG_INVALID_ARGUMENT('compareAsSets', 'array1', array1)
    );
  }
  if (!Array.isArray(array2)) {
    throw new TypeError(
      ERROR_MSG_INVALID_ARGUMENT('compareAsSets', 'array2', array2)
    );
  }
  let set1 = new Set(array1.map(key));
  let set2 = new Set(array2.map(key));
  return (
    setDifference(set1, set2).size === 0 && setDifference(set2, set1).size === 0
  );
}
...
export function compareAsLists(array1, array2, key = consistentStringify) {
  if (!Array.isArray(array1)) {
    throw new TypeError(
      ERROR_MSG_INVALID_ARGUMENT('compareAsLists', 'array1', array1)
    );
  }
  if (!Array.isArray(array2)) {
    throw new TypeError(
      ERROR_MSG_INVALID_ARGUMENT('compareAsLists', 'array2', array2)
    );
  }
