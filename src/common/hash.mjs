...
const FNV1_PRIME_MUL = (hash) =>
  (hash << 24) + (hash << 8) + (hash << 7) + (hash << 4) + (hash << 1);
const FNV1_OFFSET_BASIS = 0x811c9dc5;

const ERROR_MSG_HASH_KEY_TYPE = (fname, val) =>
  `Illegal parameter for ${fname}: key = ${val} must be a String`;
const ERROR_MSG_HASH_KEY_EMPTY = (fname) =>
  `Illegal parameter for ${fname}: key must be a non empty string`;
const ERROR_MSG_HASH_SEED = (fname, val) =>
  `Illegal parameter for ${fname}: seed = ${val} must be a SafeInteger`;
...
export function murmurHash32(key, seed = 0) {
  ...
  for (let i = 0; i < n - remainder; i += 4) {
    ...
    k1 =
      ((k1 & 0xffff) * MURMUR_C1 +
        ((((k1 >>> 16) * MURMUR_C1) & 0xffff) << 16)) &
      0xffffffff;
    k1 = (k1 << 15) | (k1 >>> 17);
    k1 =
      ((k1 & 0xffff) * MURMUR_C2 +
        ((((k1 >>> 16) * MURMUR_C2) & 0xffff) << 16)) &
      0xffffffff;
